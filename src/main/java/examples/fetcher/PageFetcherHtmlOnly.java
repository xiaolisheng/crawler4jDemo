...
public class PageFetcherHtmlOnly extends PageFetcher {
	...
	@Override
	public PageFetchResult fetchPage(WebURL webUrl) throws InterruptedException,
			IOException, PageBiggerThanMaxSizeException {
		...
		try {
			...
			synchronized (mutex) {
				...
				if (now - this.lastFetchTime < this.config
						.getPolitenessDelay()) {
					Thread.sleep(this.config.getPolitenessDelay()
			...
			String contentType = response.containsHeader("Content-Type")
					...
					: null;
			String typeStr = (contentType != null)
